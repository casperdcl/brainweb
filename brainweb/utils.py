...
__all__ = [
    ...
    "Res", "Shape",
    # probably not needed
    "noise", "seed", "toPetMmr", "LINKS"] # yapf: disable
...
LINKS = [
    ...
    '&zip_value=gnuzip&download_for_real=%5BStart+download%21%5D' for i in LINKS.split()]
RE_SUBJ = re.compile('.*(subject)([0-9]+).*')
LINKS = {RE_SUBJ.sub(r'\1_\2.bin.gz', i): i for i in LINKS}
log = logging.getLogger(__name__)


class Act:
    """careful: occasionally other bits may be set"""
    background, csf, greyMatter, whiteMatter, fat, muscle, skin, skull, vessels, aroundFat, dura, marrow = (
        i << 4 for i in range(12))
    all_labels = [
        'background', 'csf', 'greyMatter', 'whiteMatter', 'fat', 'muscle', 'skin', 'skull', 'vessels', 'aroundFat',
        'dura', 'marrow']
    ...
    def indices(cls, im, attr):
        ...
        if attr == "bone":
            ...
            matter = ["whiteMatter", "greyMatter", "skin", "csf", "fat", "muscle", "vessels", "aroundFat"]

        if matter:
            return sum((cls.indices(im, i) for i in matter[1:]), cls.indices(im, matter[0])) > 0
...
class T1(Act):
    ...
    attrs = ["whiteMatter", "greyMatter", "skin", "skull", "marrow", "bone", "csf"]
...
load_file = functools.partial(gunzip_array, shape=(362, 434, 362), dtype=np.uint16)
...
def get_files(cache_dir=None, progress=True):
    ...
    """
    files = []
    for f, url in tqdm(LINKS.items(), unit="file", desc="BrainWeb Subjects", disable=not progress):
        files.append(get_file(f, url, cache_dir=cache_dir))
    return sorted(files)


def get_mmr(cache_file, raw_data, petNoise=1.0, t1Noise=0.75, t2Noise=0.75, petSigma=1.0, t1Sigma=1.0, t2Sigma=1.0,
    ...
    if not path.exists(cache_file):
        ...
        np.savez_compressed(cache_file, PET=pet, uMap=uMap, T1=t1, T2=t2, petNoise=np.float32(petNoise),
                            t1Noise=np.float32(t1Noise), t2Noise=np.float32(t2Noise), petSigma=np.float32(petSigma),
                            t1Sigma=np.float32(t1Sigma), t2Sigma=np.float32(t2Sigma), res=np.float32(out_res))
...
def get_mmr_fromfile(brainweb_file, petNoise=1.0, t1Noise=0.75, t2Noise=0.75, petSigma=1.0, t1Sigma=1.0, t2Sigma=1.0,
                     outres="mMR", PetClass=FDG):
    """
    ...
    return get_mmr(brainweb_file.replace('.bin.gz', (f'.{PetClass.__name__}.{outres}.npz').replace('.FDG.mMR', '')),
                   dat, petNoise=petNoise, t1Noise=t1Noise, t2Noise=t2Noise, petSigma=petSigma, t1Sigma=t1Sigma,
                   t2Sigma=t2Sigma, outres=outres, PetClass=PetClass)
...
def get_label_probabilities(brainweb_file, labels=None, outres="mMR", progress=True, dtype=np.float32):
    ...
    if set(labels).difference(Act.all_labels):
        raise KeyError("labels ({}) must be in Act.all_labels ({})".format(", ".join(labels),
                                                                           ", ".join(Act.all_labels)))

    num_classes = len(labels)
    res = np.zeros((num_classes,) + tuple(out_shape), dtype=dtype)
    for i, attr in tenumerate(labels, unit="label", desc="BrainWeb labels", disable=not progress):
...
def volshow(vol, cmaps=None, colorbars=None, xlabels=None, ylabels=None, titles=None, vmins=None, vmaxs=None,
            sharex=True, sharey=True, ncols=None, nrows=None, figsize=None, frameon=True, tight_layout=1,
            fontproperties=None):
    """
    ...
    if hasattr(vol, "keys") and hasattr(vol, "values"):
        if titles is not None:
            log.warning("ignoring `vol.keys()` in favour of specified `titles`")
        else:
            titles = vol.keys()
            vol = list(vol.values())
    ...
    def plot_slice(z):
        ...
        for ax, v, cmap, cbar, xlab, ylab, tit, vmin, vmax in zip(axs, vol, cmaps, colorbars, xlabels, ylabels, titles,
                                                                  vmins, vmaxs):
            plt.sca(ax)
...
def noise(im, n, warn_zero=True, sigma=1):
  ...
  """
    if n < 0:
        ...
        if warn_zero:
            log.warning("zero noise")
        return im
    r = gaussian_filter(np.random.random(im.shape), sigma=sigma, mode='constant')
    return im * (1 + n * (2*r - 1))


def toPetMmr(im, pad=True, dtype=np.float32, outres="mMR", modes=None, PetClass=FDG):
    """
  ...
  """
    ...
    def resizeToMmr(arr):
        ...
        # arr /= arr.max()
        arr = resize(arr, new_shape, order=1, mode='constant', anti_aliasing=False, preserve_range=True).astype(dtype)
        if pad:
            arr = np.pad(arr, [(p, p + r) for (p, r) in zip(padLR.astype(int), padR.astype(int))], mode="constant")
        return arr
...
def add_lesions(im3d, dim=Res.mMR * Shape.mMR, diam=None, intensity=None, blur=None, thresh=None, PetClass=FDG):
    """
    im3d  : 3darray
...
def trim_zeros_ROI(arr, trim='fb', progress=True):
    ...
    trim = trim.upper()
    res = []
    for i in trange(arr.ndim, desc="Trimming ROI", disable=not progress, leave=False):
...
def register(src, target=None, ROI=None, target_shape=Shape.mMR, src_resolution=Res.MR, target_resolution=Res.mMR,
             method="CoM", src_offset=None, dtype=np.float32):
    """
    ...
    if target is not None:
        ...
        if np.isnan(static).sum():
            log.warning("NaNs in target reference - skipping")
        else:
            ...
            if method == "com":
                method = transform_centers_of_mass(static, np.eye(4), moving, affine_init)
            else:
